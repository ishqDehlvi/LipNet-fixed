...
class Video(object):
    ...
    def get_frames_mouth(self, detector, predictor, frames):
        ...
        for frame in frames:
            ...
            resized_img = Image.fromarray(frame).resize(new_img_shape, Image.Resampling.LANCZOS)
            # resized_img = Image.fromarray(frame).resize(new_img_shape, Image.ANTIALIAS)
            resized_img = np.array(resized_img)  # Convert back to numpy array
